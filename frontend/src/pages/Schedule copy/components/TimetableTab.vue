    ...
    <div class="tt-layout">
      <div class="tt-grid-container">
        ...
        <div class="tt-grid-body">
          ...
          <div class="tt-cells">
            ...
            <div
              v-for="(cls, idx) in currentTimetableData.classes"
              :key="idx"
              class="tt-block"
              :style="getClassStyle(cls)"
            >
              <div class="tt-block__name">
                {{ cls.courseName || cls.lectureName }}
              </div>
              <div class="tt-block__info">{{ cls.buildingRoom }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <aside class="tt-sidebar">
        <div class="search-box">
          ...
          <div class="major-filters" v-if="searchCategory === '전공'">
          ...
          <div class="major-filters" v-if="searchCategory === '교선'">
            <select v-model="selectedElectiveDomain" class="filter-select">
              <option value="">영역 전체</option>
              <option v-for="d in electiveDomainOptions" :key="d" :value="d">
                {{ d }}
              </option>
            </select>
          </div>
          
          <div class="major-filters" v-if="searchCategory === '융전'">
             <select v-model="selectedIntegratedMajorTt" class="filter-select">
               <option value="">융합전공 선택</option>
               <option v-for="m in integratedMajors" :key="m.id" :value="getTtMajor(m)">
                 {{ getMajorName(m) }}
               </option>
             </select>
          </div>

          <div class="major-filters" v-if="searchCategory === '연전'">
             <select v-model="selectedLinkedMajorTt" class="filter-select">
               <option value="">연계전공 선택</option>
               <option v-for="m in linkedMajors" :key="m.id" :value="getTtMajor(m)">
                 {{ getMajorName(m) }}
               </option>
             </select>
          </div>
        ...
        <div class="search-results">
          <div v-if="lectures.length === 0" class="empty-msg">
            <span v-if="loadingLectures">검색 중입니다...</span>
            <span v-else>검색 결과가 없습니다.</span>
          </div>
          
          <div v-else class="search-list">
             <div class="search-list__summary">
               검색 결과 {{ lectures.length }}건
             </div>
             <ul class="search-list__items">
               <li v-for="lec in lectures" :key="lec.id" class="search-item">
                 <div class="search-item__info">
                   <div class="search-item__title">
                     {{ lec.courseName }}
                     <span class="search-item__code">({{ lec.courseCode }}-{{ lec.sectionNo }})</span>
                   </div>
                   <div class="search-item__meta">
                     <span>{{ lec.professorName }}</span>
                     <span>{{ lec.credit }}학점</span>
                     <span v-for="(slot, i) in lec.slots" :key="i">
                       {{ slot.meetingDay }} {{ slot.startTime }}~{{ slot.endTime }}
                     </span>
                   </div>
                 </div>
                 <button 
...
import { useAuth } from '../../../composables/useAuth.js' // 경로 확인 필요
import { api } from '../../../api/api.js' // 경로 확인 필요
