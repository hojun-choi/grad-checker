...
# --- [Global Trade] 설정 ---
DEPARTMENT = "global_commerce"
# ---

def get_post_links(driver, old_post_ids, max_pages_to_scan):
    
    BASE_URL_TEMPLATE = "https://gtrade.ssu.ac.kr/%ed%95%99%eb%b6%80%ec%95%8c%eb%a6%bc/%ea%b3%b5%ec%a7%80%ec%82%ac%ed%95%ad/page/{page_num}/?term_id"
    ...
    try:
        for page_num in range(1, max_pages_to_scan + 1):
            ...
            for row in rows:
                ...
                # 날짜 형식 표준화 (YYYY.MM.DD 또는 YYYY-MM-DD -> YYYY-MM-DD)
                try:
