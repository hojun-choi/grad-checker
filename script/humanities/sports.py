...
def get_post_links(driver, old_post_ids, max_pages_to_scan):
    ...
    # 스포츠학부 공지사항 페이지네이션 URL 구조
    # 한글 경로: https://sports.ssu.ac.kr/자료실/신앙자료실/page/{page_num}/?term_id
    # (URL상 '신앙자료실'로 되어있으나 실제 공지사항 게시판임)
    BASE_URL_TEMPLATE = "https://sports.ssu.ac.kr/%ec%9e%90%eb%a3%8c%ec%8b%a4/%ec%8b%a0%ec%95%99%ec%9e%90%eb%a3%8c%ec%8b%a4/page/{page_num}/?term_id"
        ...
        for page_num in range(1, max_pages_to_scan + 1):
            ...
            for row in rows:
                ...
                post_data = {
                    ...
                    "title": title,
                    "author": "N/A", # 스포츠학부 목록에도 작성자 정보 없음
                    "date": date,
                    "link": link,
                }
