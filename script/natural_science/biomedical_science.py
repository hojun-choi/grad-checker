...
# --- [Biomedical Science] 설정 ---
DEPARTMENT = "biomedical_science"
# ---

def get_post_links(driver, old_post_ids, max_pages_to_scan):
    
    # 의생명시스템학부 공지사항 페이지네이션 URL 구조
    # 한글 경로: https://bioinfo.ssu.ac.kr/게시판/공지사항/page/{page_num}/?term_id
    BASE_URL_TEMPLATE = "https://bioinfo.ssu.ac.kr/%ea%b2%8c%ec%8b%9c%ed%8c%90/%ea%b3%b5%ec%a7%80%ec%82%ac%ed%95%ad/page/{page_num}/?term_id"
    
    # HTML 소스 분석에 따른 선택자 설정
    LIST_ROW_SELECTOR = ".baord_table tbody > tr"
    NUM_SELECTOR = "td:nth-of-type(1)"
    TITLE_SELECTOR = "td.title a"
    DATE_SELECTOR = "td:nth-of-type(3)" # 번호, 제목, '등록일', 조회수 순서 (파일 열 없음)
        ...
        for page_num in range(1, max_pages_to_scan + 1):
            ...
            for row in rows:
                ...
                post_data = {
                    ...
                    "title": title,
                    "author": "N/A", # 의생명시스템학부 목록에도 작성자 정보 없음
                    "date": date,
                    "link": link,
                }
