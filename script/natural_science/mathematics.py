...
def get_post_links(driver, old_post_ids, max_pages_to_scan):
    ...
    # 수학과 공지사항 페이지네이션 URL 구조
    BASE_URL_TEMPLATE = "https://math.ssu.ac.kr/?page_id=977&paged={page_num}"
    
    # HTML 소스 분석에 따른 선택자 설정
    # class="table_wrap baord_table" (HTML 원본의 baord 오타 반영)
    LIST_ROW_SELECTOR = ".baord_table tbody > tr"
    NUM_SELECTOR = "td:nth-of-type(1)"
    TITLE_SELECTOR = "td.title a"
    DATE_SELECTOR = "td:nth-of-type(4)" # 번호, 제목, 파일, '날짜', 조회수 순임
